...
const Database = (_props: TListComponentProps) => {
  ...
  useEffect(() => {
    assetsApplicationsVersion({ app_type: 'database' })
      .pipe(tap((res: WebResponse<any>) => setVersion(res.getItems()?.map((v: string) => ({ label: v, value: v })))))
      .subscribe();
    assetsApplicationsTargets({ app_type: 'database' })
  ...
  const {
    ...
    ({ current, pageSize }) => {
      ...
      return assetsApplications({ ...filters, app_type: 'database' }, pageParams)
        .pipe(
      ...
      <div className="mb12">
        <FilterContext.Provider value={{ ...data }}>
          <div className="data-list-case-bar">
            <span className="headTit">{translations.clusterGraphList_database}</span>
            <TzFilter />
          </div>
      ...
      <TzTable
        ...
        onRow={(record) => {
          return {
            onClick: () => {
              navigate(
                `${Routes.ClustersOnlineVulnerabilitiesDetails}?type=database&NSName=${record.namespace}&name=${record.resourceName}&ClusterID=${record?.clusterKey}`,
              );
            },
          };
        }}
